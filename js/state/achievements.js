...
export async function init(){
    try {
        let json = null;
        try {
            if (StaticData && typeof StaticData.getConfig === 'function') {
                json = StaticData.getConfig('achievements');
            }
        } catch {}
        if (!json) {
...
function bump(id, delta){
    ...
    if (done) {
        try {
            ...
            callIfExists('UI', 'showToast', ['gold', `Достижение "${name}" получено! ${icon}`], undefined);
        } catch {}
    }
}

export function onUnitKilled(monsterTypeId){
...
export function onCurrencyEarned(currencyId, amount){
...
export const Achievements = { init, getAll, getById, onUnitKilled, onCurrencyEarned, clearAll };
